...
<div class="form-group">
<label>等级</label><br><select class="form-control" name="level" default="'.$row['level'].'"><option value="1">普通商户</option><option value="2">黄金vip</option><option value="3">钻石vip</option></select>
</div>
<label>等级到期时间</label><br>
<div>
<input type="text" class="form-control" name="level_endtime" value="'.date('Y-m-d H:i:s',$row['level_endtime']).'" required>
</div><br />
<div class="form-group">
<label>安全码</label><br>
<input type="text" class="form-control" name="safecode" value="'.$row['safecode'].'" placeholder="可留空" >
</div><br />
...
<div class="form-group">
<label>安全码</label><br>
<input type="text" class="form-control" name="safecode" value="'.$row['safecode'].'" >
</div><br />
...
$active=$_POST['active'];
$level=$_POST['level'];
$level_endtime=strtotime($_POST['level_endtime']);
$safecode=$_POST['safecode'];
if($account==NULL or $username==NULL){
showmsg('保存错误,请确保加*项都不为空!',3);
} else {
$key = random(32);
$sds=$DB->exec("INSERT INTO `pay_user` (`key`, `account`, `username`, `money`, `url`, `addtime`, `type`, `settle_id`, `email`, `qq`, `rate`, `active`,`level`,`level_endtime`,`safecode`) VALUES ('{$key}', '{$account}', '{$username}', '{$money}', '{$url}', '{$date}', '{$type}', '{$settle_id}', '{$email}', '{$qq}', '{$rate}', '{$active}', '{$level}', '{$level_endtime}', '{$safecode}')");
$pid=$DB->lastInsertId();
if($sds){
...
$sql="update `pay_user` set `level_endtime`='{$level_endtime}',`account` ='{$account}',`username` ='{$username}',`money` ='{$money}',`url` ='{$url}',`type` ='$type',`settle_id` ='$settle_id',`email` ='$email',`qq` ='$qq',`rate` ='$rate',`active` ='$active',`level`='$level',`safecode`='$safecode' where `id`='$id'";
if($_POST['resetkey']==1){
	$key = random(32);
...
?>
      <div class="table-responsive">
        <table class="table table-striped">
          <thead><tr><th>商户号</th><th>密钥</th><th>余额</th><th>结算账号/姓名</th><th>域名/添加时间</th><th>等级</th><th>状态</th><th>操作</th></tr></thead>
          <tbody>
<?php
$pagesize=30;
...
elseif($res['level']==3){
  $res['levels']='钻石会员';
    $res['lvcol']='red';}
echo '<tr><td><b>'.$res['id'].'</b></td><td>'.$res['key'].'</td><td>'.$res['money'].'</td><td>'.($res['settle_id']==2?'<font color="green">WX:</font>':null).($res['settle_id']==3?'<font color="green">QQ:</font>':null).$res['account'].'<br/>'.$res['username'].'</td><td>'.$res['url'].'<br/>'.$res['addtime']
  .'</td><td><font color='.$res['lvcol'].'>'.$res['levels'].'</font></td><td>'.($res['active']==1?'<font color=green>正常</font>':'<font color=red>封禁</font>').'</td><td><a href="./ulist.php?my=edit&id='.$res['id'].'" class="btn btn-xs btn-info">编辑</a>&nbsp;<a href="./ulist.php?my=delete&id='.$res['id'].'" class="btn btn-xs btn-danger" onclick="return confirm(\'你确实要删除此商户吗？\');">删除</a></td></tr>';
}
?>
          </tbody>
        </table>
