...
function subText($content, $lengh = 100){
    ...
    if(mb_strlen($text) > $lengh){

        $text = mb_substr($text,0,$lengh,'utf-8');
...
function getTemplates(){
    $templates = count(getTemplate());
    return $templates;
}
function getTemplate(){
    $template = getDir(ROOT.'template');
    return $template;
}
function getDir($dir) {
    $dirArray[]=NULL;
...
function checkmobile() {
	...
	$ualist = array('android', 'midp', 'nokia', 'mobile', 'iphone', 'ipod', 'blackberry', 'windows phone','Safari');
...
function processOrder($srow,$notify=true){
	global $DB,$conf;
	$userrow=$DB->query("SELECT * FROM pay_user WHERE id='{$srow['pid']}'")->fetch();
	if($userrow['level']==1)
		if ($srow['type'] == 'wxpay') $rat = $conf['money_rate_wxpay'];
	    if ($srow['type'] == 'qqpay') $rat = $conf['money_rate_qqpay'];
	    if ($srow['type'] == 'alipay') $rat = $conf['money_rate_alipay'];
	if($userrow['level']==2)
		$rat=$conf['money_rate_vip'];
	if($userrow['level']==3)
		$rat=$conf['money_rate_svip'];
	$rate=$userrow['rate'];
	if(!(!empty($rate) && $rate>=$rat && $rate<100)){
		$rate = $rat;
	}
	$addmoney=sprintf('%.3f',$srow['money']*$rate/100);
