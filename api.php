...
elseif($act=='apply')
{
	...
	$row=$DB->query("SELECT * FROM panel_user WHERE token='{$token}' limit 1")->fetch(PDO::FETCH_ASSOC);
	if($row && $row['active']==1){
		$type=0;
		$key = random(32);
...
elseif($act=='query')
{
	...
	$row=$DB->query("SELECT * FROM pay_user WHERE id='{$pid}' limit 1")->fetch(PDO::FETCH_ASSOC);
	if($row){
		if($key==$row['key']){
			...
			$result=array_merge(array("code"=>1,"pid"=>$pid,"key"=>$key,"settle_money"=>$conf['settle_money'],"settle_fee"=>$conf['settle_fee'],"money_rate"=>$conf['money_rate'],"orders"=>$orders,"order_today"=>$order_today,"order_lastday"=>$order_lastday,"settle_money"=>$settle_money),$row);
		}else{
...
elseif($act=='order')
{
	...
	if($row){
		if($key==$row['key']){
            ...
            $row=$DB->query("SELECT * FROM pay_order WHERE out_trade_no='{$out_trade_no}' limit 1")->fetch(PDO::FETCH_ASSOC);
            if($row){
