...
if ($type == 'alipay') {
	if ($conf['alipay_api'] == 4) {
		...
		$parameter = array("service" => $alipay_service, "partner" => trim($alipay_config['partner']), "seller_id" => trim($alipay_config['partner']), "payment_type" => "1", "notify_url" => 'http://' . $conf['local_domain'] . '/alipay_notify.php', "return_url" => 'http://' . $_SERVER['HTTP_HOST'] . '/alipay_return.php', "out_trade_no" => $trade_no, "subject" => $row['name'], "total_fee" => $row['money'], "_input_charset" => strtolower('utf-8'));
		if (checkmobile() == true) {
			$parameter['app_pay'] = "Y";
		}
