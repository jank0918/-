...
<strong>自助申请商户</strong>
    <?php
    $issetn = $DB->query("SELECT * FROM `pay_notice` WHERE `type` = 1")->fetch();
    if ($issetn['id'] != ''){
    ?>
    <div class="panel panel-default">

        <?php
        $ns = $DB->query("SELECT * FROM `pay_notice` WHERE `type` = 1 order by id desc limit 5");
        ?>
        <div class="list-group">
            <?php
            while ($row = $ns->fetch()){
                echo '<div class="list-group-item">'.$row['content'].'</div>';
            }
            ?>
        </div>
...
<div class="list-group-item">
<input type="text" name="sitename" value="<?php echo $conf['web_name']?>" class="form-control no-border" required style="display:none">
...
$(document).ready(function(){
	...
	$("#submit").click(function(){
		...
		$.ajax({
			...
			data : {type:type,account:account,username:username,url:url,email:email,phone:phone,code:code,sitename:sitename},
			dataType : 'json',
			success : function(data) {
