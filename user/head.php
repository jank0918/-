...
if ($conf['safe_switch'] == 1){
    if (!empty($userrow['safecode'])){
        if ($islogin3 == 1){}else{
            @header('Content-Type: text/html; charset=UTF-8');
            exit("<script language='javascript'>alert('需要安全码二次验证！');window.location.href='safe.php';</script>");
        }
    }
}
?>
        ...
        <div class="navi-wrap">
          ...
          <nav ui-nav class="navi clearfix">
            <ul class="nav">
              ...
              <li>
                <a href="./apply.php">
                  <i class="glyphicon glyphicon-fire icon text-danger-dk"></i>
          <b class="label bg-danger pull-right">N</b>
                  <span class="font-bold">申请提现</span>
                </a>
              </li>
              <?php }else{?> <?php }?>
              <li class="line dk"></li>
